...
const int IDENTIFI_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString IDENTIFI_IPC_PREFIX("identifi:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(IDENTIFI_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(IDENTIFI_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(IDENTIFI_IPC_CONNECT_TIMEOUT);
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click identifi: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start identifi: click-to-pay handler");
    else
...
{
    // clicking on identifi: URLs creates FileOpen events on the Mac:
