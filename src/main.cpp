...
uint256 hashGenesisBlock("0x000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26e");
...
void ThreadScriptCheck() {
    RenameThread("identifi-scriptch");
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("IdentifiMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("IdentifiMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static IdentifiMiner(CWallet *pwallet)
{
    printf("IdentifiMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("identifi-miner");
    ...
    try { loop {
        ...
        printf("Running IdentifiMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("IdentifiMiner terminated\n");
        throw;
    }
}

void GenerateIdentifis(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&IdentifiMiner, pwallet));
}
